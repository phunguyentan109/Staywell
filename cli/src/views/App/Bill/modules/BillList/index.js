...
function BillList({ contracts, roomId }) {
  const [contractIds, setBillIds] = useState([])
  const [perPage, setPerPage] = useState(5)

  const hdSelect = (checked, id) => {
    if (checked) return setBillIds(prev => [...prev, id])
    setBillIds(prev => prev.filter(v => v !== id))
  }

  const hdSelectAll = checked => {
    setBillIds(!checked ? [] : _.cloneDeep(samples20).splice(0, perPage).map(c => c._id))
  }
