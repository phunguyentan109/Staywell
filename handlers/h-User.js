...
exports.openRegistration = async(req, res, next) => {
  try {
    ...
    if (foundUser) {
      let { tokens, usedTokens } = await foundUser.getAnonymous()
      tokens = tokens.map(t => t.token)
      usedTokens = usedTokens.map(t => t.token)
      tokens.push(registrationToken)
      foundUser.anonymous = await foundUser.setAnonymous({ tokens, usedTokens })
...
exports.closeRegistration = async(req, res, next) => {
  try {
    ...
    if (foundUser && foundUser.anonymous) {
      let { tokens, usedTokens } = foundUser.anonymous

      // Store non-expired tokens to prevent renter from reusing
      if (_.includes(tokens, req.params.token)) {
        tokens = tokens.filter(t => t !== req.params.token)
        usedTokens.push(req.params.token)
        foundUser.anonymous = foundUser.setAnonymous({ tokens, usedTokens })
        await foundUser.save()
      }
    }
    return res.status(200).json({ success: true })
